 ...
 *
 * Defines the board's CAN node identity, TCAN3403 transceiver pins,
 * magnetorquer coil pin mapping (PC2-PC5, PWM where a timer AF is
 * available, GPIO-only otherwise), watchdog and MCU role-select pins,
 * default PWM frequency, and actuation safety limits (max on-time, max
 * duty cycle).
 *
 * Pin assignments:
